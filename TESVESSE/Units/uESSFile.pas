...
uses
  Windows, Graphics, ZLib, VirtualTrees;
...
type
  ...
  TPluginInfo = packed record
    pluginCount: uint8;
    plugins: array of pwstring; //pluginCount
  end;
  ...
  PChangeForm = ^TChangeForm;
  TChangeForms = array of	PChangeForm;
  PChangeForms = ^TChangeForms;

  TUnknown3Table = packed record
    count: uint32;
    unknown: array of pwstring; //count
  end;
  ...
  PFormIDArray = ^TFormIDArray; {$IF Defined(USEREGION)}{$ENDREGION 'ESS Types'}{$IFEND}
  ...
  TESSFile = packed record
    ...
    globalDataTable1:	array of PGlobalData; //Types 0 to 8. fileLocationTable.globalDataTable1Count
    globalDataTable2:	array of PGlobalData;	//Types 100 to 114. fileLocationTable.globalDataTable2Count
    changeForms: array of	PChangeForm; //fileLocationTable.changeFormCount
    globalDataTable3: array	of PGlobalData; //Types 1000 to 1005. fileLocationTable.globalDataTable3Count
    formIDArrayCount:	uint32;
    formIDArray: array of	PFormIDArray;
    visitedWorldspaceArrayCount:	uint32;
    visitedWorldspaceArray: array of	uint32; //visitedWorldspaceArrayCount
  ...
  {$IF Defined(USEREGION)}{$REGION 'Papyrus Structures'}{$IFEND}
  TString = packed record
    sName: string;
  end;
  PString = ^TString;
  TScriptParameters = packed record
    iName, iSType: Word;
  end;
  PScriptParameters = ^TScriptParameters;
  TScript = packed record
    iName, iSType: Word;
    iParameterCount: Integer;
    yParameters: array of PScriptParameters;
  end;
  PScript = ^TScript;
  TInstance = packed record
    iID: Integer;
    iScript: Word;
    iUnknow1, iUnknow2: Word;
    rRef: TRefID;
    iUsed: Byte;
  end;
  PInstance = ^TInstance;
  ...
  TArray = packed record
    iID: Integer;
    iType: Byte;
    iReference, iMemberCount: Word;
  end;
  PArray = ^TArray;
  TActive = packed record
    iID: Integer;
    iUsed: Byte;
  end;
  PActive = ^TActive;
  ...
  TData = packed record
    ...
    iFlag: Byte;
    iReference: Word;
    iData1, iData2, iMemberCount: Integer;
    yMembers: array of PMember;
  end;
  PData = ^TData;
  TArrayData = packed record
    iID: Integer;
    yMembers: array of PMember;
  end;
  ...
  TFunctionParam = record
    iName, iType: uint16;  //String table reference
  end;
...
//  * = uint32 extra parameters count
  TOpcodeData = record
    iOpcode: uint8;
    yParameter: array of TParameter;
  end;

  TStackframe = record
    iVariablecount: uint32;
    bFlag: uint8;
    bFunctionType: uint8;
    iScriptName, iScriptBaseName, iEvent: uint16; //String table reference
    iStatus: uint16; //String table reference. Appears only if (flag and 0x01) == 0x00, and function type is 0x00
    iOpcodeVersion, iOpcodeMinorVersion: uint8;
    iReturnType, iFunctionDocString: uint16; //String table reference
    iFunctionUserFlags: uint32;
    iFunctionFlags: uint8;
    iFunctionParameterCount: uint16;
    uFunctionParam:	array of TFunctionParam;
    iFunctionLocalsCount: uint16;
    yFunctionLocal: array of TFunctionParam;
    iOpcodeCount: uint16;	
    yOpcodeData: array of TOpcodeData;
    iUnknown3: uint32;
    tUnknown: array of TMember;
  end;

  TActiveData = packed record
    iID: uint32;
    iMajorVersion, iMinorVersion: uint8;
    tUnknown: TMember;
    bFlag, bUnknownByte: uint8;
    iUnknown2: uint32;    //Appears only if (flag and 0x01) == 0x01
    iUnknown3: uint8;
    tUnknown4: record  //Appears only if unknown3 is between 1 - 3
                 sName: string;
                 tVariable: TMember;
                 tRef: TRefID;
                 iStr: uint16;
...
//  SceneResults        3 bytes refId
    ...
    iStackframecount: uint32;
    yStackframe: array of TStackframe;
    unknown5: uint8; //Appears only if Stackframecount is not NULL
    yData: TBytes;
  end;
  PActiveData = ^TActiveData;

  TFunctionMessageData = record
    unknown: uint8;	
    iScriptName: uint16; //String table reference
    iEvent: uint16;	//String table reference
    tUnknow: TMember;
    iVariableCount: uint32;	
    yUnknown:	array of TMember;
  end;
  
  TFunctionMessage = record
    unknown: uint8; //If value >2, no id, flag and message are present
    iID: uint32;	
    iFlag: uint8;	
    tMessageData:	TFunctionMessageData;	//Appears only if flag >0
  end;
  PFunctionMessage = ^TFunctionMessage;
  
  TSuspendedStack = record
    iID: uint32;	
    iFlag: uint8;	
    tMessageData:	TFunctionMessageData;	//Appears only if flag >0
  end;
  PSuspendedStack = ^TSuspendedStack;
  {$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}

  tPapyrus = packed record
    iVersion: Word;
    iStringCount: Integer;
    yStrings: array of PString;
    iScriptCount: Integer;
    yScripts: array of PScript;
    iInstanceCount: Integer;
    yInstances: array of PInstance;
    iReferenceCount: Integer;
    yReferences: array of PReference;
    iArrayCount: Integer;
    yArrays: array of PArray;
    iPapyrusRuntime: Integer;
    iActiveCount: Integer;
    yActives: array of PActive;
    yInstanceData, yReferenceData: array of PData;
    yArrayData: array of PArrayData;
    yActiveData: array of PActiveData;
    iFunctionMessageCount: uint32;
    yFunctionMessages: array of PFunctionMessage;
    iSuspendedStackCount1: uint32;
    ySuspendedStacks1: array of PSuspendedStack;
    iSuspendedStackCount2: uint32;
    ySuspendedStacks2: array of PSuspendedStack;
    iUnknown1, iUnknown2: uint32;
    iUnknownCount: uint32;
    tUnknown: array of uint32;
    iQueuedUnbindCount: uint32;
    yQueuedUnbinds: array of record
      iUnk1, iUnk2: uint32;
    end;
    saveFileVersion: int16;
    {$IF Defined(USEREGION)}{$REGION 'Recognize work'}{$IFEND}

    iArrayCount1: uint32;
    yArray1: array of record
      iUnk1, iUnk2: uint32;
    end;

    iArrayCount1a: uint32;

    iArrayCount2: uint32;
    yArray2: array of record
      iUnk1, iUnk2: uint32;
    end;

    iArrayCount3: uint32;
    yArray3: array of record
      iType: uint8;
      iStr1, iUnk1, iStr2: uint16;
      iUnk2: uint32;
      iFlag: uint16;
      tRef: TRefID;
    end;
    iArrayCount4: uint32;
    yArray4: array of record
      iStr1, iUnk1: uint16;
      iUnk2: uint8;
      iStr2: uint16;
      iUnk3: uint32;
    ...
    iScriptListCount: uint32;
    yScriptList: array of string;

    iArrayCount4a: uint32;

    iArrayCount4b: uint32;
    yArray4b: array of record
      iUnk1: uint8;
      iUnk2, iUnk3: uint16;
      tRef1, tRef2, tRef3, TRef4: TRefID;
    end;

    iArrayCount4c: uint32;
    yArray4c: array of record
      iFlag: uint8;
      iUnk1: uint32;
      tRef1, tRef2: TRefID;
      iUnk2, iUnk3: uint32;
      iUnk4, iUnk5: uint16;
      iUnk6, iUnk7, iUnk8: uint32;
      iUnk9: uint16;
    end;

    iArrayCount4d: uint32;

    iArrayCount5: uint32;
    yArray5: array of record
      iUnk1, iUnk2: uint16;
      tUnk3, tUnk4, tUnk5: TRefID;
      iUnk6: uint16;
    end;

    iArrayCount6: uint32;
    yArray6: array of record
      iUnk1, iFlag: uint16;
      tRef: TRefID;
    end;

    iArrayCount7: uint32;
    yArray7: array of record
      iUnk1, iFlag: uint16;
      tRef: TRefID;
    end;

    iArrayCount8: uint32;
    yArray8: array of record
      iUnk1, iType: uint16;
      tRef: TRefID;
      iCount1: uint32;
      iCount2: uint32;
      yRef: array of TRefID;
    end;

    iArrayCount9: uint32;
    yArray9: array of uint32;

    iArrayCount10: uint32;
    yArray10: array of record
      tArrayRef: TRefID;
      iArrayCount: uint32;
      yArray: array of record
        sName: string;
        iCount1: uint32;
        yData1: array of record
          iCount: uint32;
          yData: array of record
            iStr, iFlag: uint16;
          end;
        end;
        iCount2: uint32;
        yData2: array of record
          iUnk, iFlag: uint16;
          tRef: TRefID;
        end;
      end;
    ...
    iArrayCount11: uint32;
    yArray11: array of record
      iUnk1: uint32;
      tRef: TRefID;
      iUnk2: uint8;
    end;

    iArrayCount12: uint32;
    yArray12: array of record
      iUnk1: uint8;
      iUnk2: uint16;
      iCount: uint32;
      yData: array of record
    ...
    iArrayCount13: uint32;
    yArray13: array of record
      tRef: TRefID;
      iCount1: uint32;
      yData1: array of uint32;
      iCount2: uint32;
      yData2: array of uint32;
      iCount3: uint32;
      yData3: array of uint32;
      iUnk: uint8;
    end;

    iArrayCount14: uint32;
    yArray14: array of uint32;
    iArrayCount15: uint32;
    {$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}
    iOther: integer;
    yOther: TBytes;
    bSave: Boolean;
    pPapyrus: PVirtualNode;
  end;
  PPapyrus = ^TPapyrus;
...
function BytesToString(var v: TBytes): string;
function BytesToHex(var v: TBytes): string;
function MidBytesToHex(var v: TBytes; iStart: integer; iLength: integer = 16): string;
...
function Read_w32string(var Buf: TBytes; var FromPos: Integer): string;
function Read_vsval(var Buf: TBytes; var FromPos: Integer): vsval;
function Read_variable(var Buf: TBytes; var FromPos: Integer): TMember;

function SearchPatern(var Buf: TBytes; FromPos: Integer; var Patern: TBytes): Integer;
procedure Read_data(var Buf: TBytes; var FromPos: Integer; Size: Integer; var Data: TBytes);
{$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}

{$IF Defined(USEREGION)}{$REGION 'Write buffer function'}{$IFEND}
procedure Write_uint32(var Buf: TBytes; Value: uint32; var FromPos: Integer);
procedure Write_uint16(var Buf: TBytes; Value: uint16; var FromPos: Integer);
procedure Write_uint8(var Buf: TBytes; Value: uint8; var FromPos: Integer);
procedure Write_int32(var Buf: TBytes; Value: int32; var FromPos: Integer);
procedure Write_int16(var Buf: TBytes; Value: int16; var FromPos: Integer);
procedure Write_int8(var Buf: TBytes; Value: int8; var FromPos: Integer);
procedure Write_float32(var Buf: TBytes; Value: float32; var FromPos: Integer);
procedure Write_float64(var Buf: TBytes; Value: float64; var FromPos: Integer);
procedure Write_RefID(var Buf: TBytes; var Value: TRefID; var FromPos: Integer);
procedure Write_wstring(var Buf: TBytes; Value: wstring; var FromPos: Integer);
procedure Write_w32string(var Buf: TBytes; Value: string; var FromPos: Integer);
procedure Write_vsval(var Buf: TBytes; Value: Cardinal; var FromPos: Integer);
procedure Write_variable(var Buf: TBytes; var Value: TMember; var FromPos: Integer);
procedure Write_data(var Buf: TBytes; var FromPos: Integer; var Data: TBytes);
{$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}
...
function BytesCompare(var Data1, Data2: TBytes): integer;
...
procedure ClearActiveData(var Data: TActiveData);
procedure ClearPapyrus(var Papyrus: tPapyrus);
function GetName(iName: integer; var sPapytus: TPapyrus): string;

function GetDataType(iType: Byte): string;
...
begin
  Result := '';
  if (iStart + iLength) < Length(v) then
    iLength := iStart + iLength
  else
    iLength := Length(v);
  for i := iStart to iLength - 1 do
  begin
    Result := Result + IntToHex(v[i], 2);
    if i <> (iLength - 1) then
      Result := Result + ' ';
...
begin
  Result := '';
  if (FromPos + SizeOf(Size)) <= Length(Buf) then
  begin
    Move(Buf[FromPos], Size, SizeOf(Size));
    FromPos := FromPos + SizeOf(Size);
    SetLength(Result, Size);
    if (FromPos + Integer(Size)) <= Length(Buf) then
    begin
      if Size > 0 then
      begin
        Move(Buf[FromPos], Result[1], Size);
        FromPos := FromPos + Integer(Size);
      end;
      Exit;
    end
  end;
...
begin
  ZeroMemory(@Result, SizeOf(Result));
  if (FromPos + SizeOf(Result.iType)) <= Length(Buf) then
  begin
    Move(Buf[FromPos], Result.iType, SizeOf(Result.iType));
    FromPos := FromPos + SizeOf(Result.iType);
    case Result.iType of
      1, 11:
        if (FromPos + SizeOf(Result.iReference)) <= Length(Buf) then
        begin
          Move(Buf[FromPos], Result.iReference, SizeOf(Result.iReference));
          FromPos := FromPos + SizeOf(Result.iReference);
          if (FromPos + SizeOf(Result.iData)) <= Length(Buf) then
          begin
            Move(Buf[FromPos], Result.iData, SizeOf(Result.iData));
            FromPos := FromPos + SizeOf(Result.iData);
          end
          else
            FromPos := 0;
        end
          ...
          FromPos := 0;
      2:
        if (FromPos + SizeOf(w)) <= Length(Buf) then
        begin
          Move(Buf[FromPos], w, SizeOf(w));
          Result.iData := w;
          FromPos := FromPos + SizeOf(w);
...
begin
  Result := -1;
  if Length(Patern) > 0 then
    while (FromPos + Length(Patern)) <= Length(Buf) do
    begin
      Data := @Buf[FromPos];
      b := True;
      for i := 0 to Length(Patern) - 1 do
      begin
        if Patern[i] <> Data^ then
        begin
          b := False;
          Break;
        end;
...
procedure Read_data(var Buf: TBytes; var FromPos: Integer; Size: integer; var Data: TBytes);
begin
  if (FromPos + Size) > Length(Buf) then
    Size := Length(Buf) - FromPos;
  SetLength(Data, Size);
  if Size > 0 then
    Move(Buf[FromPos], Data[0], Size);
  FromPos := FromPos + Size;  
end;
{$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}
...
procedure Write_uint32(var Buf: TBytes; Value: uint32; var FromPos: Integer);
begin
  if (FromPos + SizeOf(Value)) > Length(Buf) then
    SetLength(Buf, FromPos + SizeOf(Value));
  Move(Value, Buf[FromPos], SizeOf(Value));
  FromPos := FromPos + SizeOf(Value);
end;
...
procedure Write_wstring(var Buf: TBytes; Value: wstring; var FromPos: Integer);
begin
  if (FromPos + Value.w + SizeOf(Value.w)) > Length(Buf) then
    SetLength(Buf, FromPos + Value.w + SizeOf(Value.w));
  Move(Value.w, Buf[FromPos], SizeOf(Value.w));
  FromPos := FromPos + SizeOf(Value.w);
  if Value.w > 0 then
    Move(Value.s[0], Buf[FromPos], Value.w);
  FromPos := FromPos + Value.w;
end;
...
begin
  Size := Length(Value);
  if (FromPos + Length(Value) + SizeOf(Size)) > Length(Buf) then
    SetLength(Buf, FromPos + Length(Value) + SizeOf(Size));
  Move(Size, Buf[FromPos], SizeOf(Size));
  FromPos := FromPos + SizeOf(Size);
  if Size > 0 then
    Move(Value[1], Buf[FromPos], Size);
  FromPos := FromPos + Integer(Size);
end;
...
var
  ...
  c: Cardinal;
begin
  if (Value and $FFFFFFC0) = 0 then
  begin
    b := Value shl 2;
    if (FromPos + SizeOf(b)) > Length(Buf) then
      SetLength(Buf, FromPos + SizeOf(b));
    Move(b, Buf[FromPos], SizeOf(b));
    FromPos := FromPos + SizeOf(b);
  end
  else
    if (Value and $FFFFC000) = 0 then
    begin
      w := (Value shl 2) or 1;
      if (FromPos + SizeOf(w)) > Length(Buf) then
        SetLength(Buf, FromPos + SizeOf(w));
      Move(w, Buf[FromPos], SizeOf(w));
      FromPos := FromPos + SizeOf(w);
    end
    else
    begin
      c := (Value shl 2) or 2;
      if (FromPos + SizeOf(c)) > Length(Buf) then
        SetLength(Buf, FromPos + SizeOf(c));
      Move(c, Buf[FromPos], SizeOf(c));
      FromPos := FromPos + SizeOf(c);
    end;
end;
...
var
  w: Word;
begin
  if (FromPos + SizeOf(Value.iType)) > Length(Buf) then
    SetLength(Buf, FromPos + SizeOf(Value.iType));
  Move(Value.iType, Buf[FromPos], SizeOf(Value.iType));
  FromPos := FromPos + SizeOf(Value.iType);
  case Value.iType of
    1, 11:
    begin
      if (FromPos + SizeOf(Value.iReference)) > Length(Buf) then
        SetLength(Buf, FromPos + SizeOf(Value.iReference));
      Move(Value.iReference, Buf[FromPos], SizeOf(Value.iReference));
      FromPos := FromPos + SizeOf(Value.iReference);
      if (FromPos + SizeOf(Value.iData)) > Length(Buf) then
        SetLength(Buf, FromPos + SizeOf(Value.iData));
      Move(Value.iData, Buf[FromPos], SizeOf(Value.iData));
      FromPos := FromPos + SizeOf(Value.iData);
    end;
    2:
    begin
      if (FromPos + SizeOf(w)) > Length(Buf) then
...
procedure Write_data(var Buf: TBytes; var FromPos: Integer; var Data: TBytes);
begin
  if (FromPos + Length(Data)) > Length(Buf) then
    SetLength(Buf, FromPos + Length(Data));
  if Length(Data) > 0 then
    Move(Data[0], Buf[FromPos], Length(Data));
  FromPos := FromPos + Length(Data);
end;
{$IF Defined(USEREGION)}{$ENDREGION}{$IFEND}
...
begin
  case _Type of
    ...
    104: Result := 'Unknown 104';
    105: Result := 'Detection Manager';
    106: Result := 'Location MetaData';
    107: Result := 'Quest Static Data';
    108: Result := 'Story Teller';
    109: Result := 'Magic Favorites';
    110: Result := 'PlayerControls';
    111: Result := 'Story Event Manager';
    112: Result := 'Ingredient Shared';
    113: Result := 'Menu Controls';
    114: Result := 'Menu Topic Manager';
  ...
  begin
    ...
    with pluginInfo do
      ...
      begin
        ...
        Dispose(plugins[Length(plugins) - 1]);
        SetLength(plugins, Length(plugins) - 1);
      end;
    while Length(globalDataTable1) > 0 do
    begin
      SetLength(globalDataTable1[Length(globalDataTable1) - 1]^.data^, 0);
      Dispose(globalDataTable1[Length(globalDataTable1) - 1]^.data);
      Dispose(globalDataTable1[Length(globalDataTable1) - 1]);
    ...
    begin
      ...
      Dispose(globalDataTable2[Length(globalDataTable2) - 1]^.data);
      Dispose(globalDataTable2[Length(globalDataTable2) - 1]);
    ...
    begin
      ...
      Dispose(changeForms[Length(changeForms) - 1]^.data);
      Dispose(changeForms[Length(changeForms) - 1]);
    ...
    begin
      ...
      Dispose(globalDataTable3[Length(globalDataTable3) - 1]^.data);
      Dispose(globalDataTable3[Length(globalDataTable3) - 1]);
      SetLength(globalDataTable3, Length(globalDataTable3) - 1);
    end;
    while Length(formIDArray) > 0 do
    begin
      Dispose(formIDArray[Length(formIDArray) - 1]);
    ...
    with unknown3Table do
      ...
      begin
        ...
        Dispose(unknown[Length(unknown) - 1]);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Inc(Result); if not Get_uint16(F, playerSex) then Exit;
        Inc(Result); if not Get_float32(F, playerCurExp) then Exit;
        Inc(Result); if not Get_float32(F, playerLvlUpExp) then Exit;
      ...
      begin
        ...
        for i := 0 to pluginCount - 1 do
          begin Inc(Result); New(plugins[i]); if not Get_wstring(F, plugins[i]^) then Exit; end;
      end;
      ...
      Inc(Result); if not Get_uint32(F, visitedWorldspaceArrayCount) then Exit;
      SetLength(visitedWorldspaceArray, visitedWorldspaceArrayCount);
      for i := 0 to visitedWorldspaceArrayCount - 1 do
        begin Inc(Result); if not Get_uint32(F, visitedWorldspaceArray[i]) then Exit; end;
      ...
      begin
        ...
        for i := 0 to Count - 1 do
          begin Inc(Result); New(unknown[i]); if not Get_wstring(F, unknown[i]^) then Exit; end;
      end;
    end;
    Result := 0;
  finally
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Inc(Result); BlockWrite(f, playerSex, SizeOf(playerSex)); LastIOState := IOResult; if LastIOState <> 0 then Exit;
        Inc(Result); BlockWrite(f, playerCurExp, SizeOf(playerCurExp)); LastIOState := IOResult; if LastIOState <> 0 then Exit;
        Inc(Result); BlockWrite(f, playerLvlUpExp, SizeOf(playerLvlUpExp)); LastIOState := IOResult; if LastIOState <> 0 then Exit;
      ...
      visitedWorldspaceArrayCount := Length(visitedWorldspaceArray);
      Inc(Result); BlockWrite(f, visitedWorldspaceArrayCount, SizeOf(visitedWorldspaceArrayCount)); LastIOState := IOResult; if LastIOState <> 0 then Exit;
      for i := 0 to visitedWorldspaceArrayCount - 1 do
        begin Inc(Result); BlockWrite(f, visitedWorldspaceArray[i], SizeOf(visitedWorldspaceArray[i])); LastIOState := IOResult; if LastIOState <> 0 then Exit; end;
...
begin
  with Data do
  begin
    while Length(yStackframe) > 0 do
    begin
      with yStackframe[Length(yStackframe) - 1] do
      begin
        SetLength(uFunctionParam, 0);
        SetLength(yFunctionLocal, 0);
        while Length(yOpcodeData) > 0 do
        begin
          with yOpcodeData[Length(yOpcodeData) - 1] do
            SetLength(yParameter, 0);
          SetLength(yOpcodeData, Length(yOpcodeData) - 1);
        end;
        SetLength(tUnknown, 0);
      end;
      SetLength(yStackframe, Length(yStackframe) - 1);
    end;
    SetLength(yData, 0);
  end;
end;
...
begin
  ...
  begin
    SetLength(yOther, 0);
    SetLength(yArray14, 0);
    while Length(yArray13) > 0 do
    begin
      SetLength(yArray13[Length(yArray13) - 1].yData1, 0);
      SetLength(yArray13[Length(yArray13) - 1].yData2, 0);
      SetLength(yArray13[Length(yArray13) - 1].yData3, 0);
      SetLength(yArray13, Length(yArray13) - 1);
    end;
    while Length(yArray12) > 0 do
    begin
      SetLength(yArray12[Length(yArray12) - 1].yData, 0);
      SetLength(yArray12, Length(yArray12) - 1);
    end;
    SetLength(yArray11, 0);
    while Length(yArray10) > 0 do
    begin
      with yArray10[Length(yArray10) - 1] do
      while Length(yArray) > 0 do
      begin
        with yArray[Length(yArray) - 1] do
        begin
          sName := '';
          while Length(yData1) > 0 do
          begin
            with yData1[Length(yData1) - 1] do
              SetLength(yData, 0);
            SetLength(yData1, Length(yData1) - 1);
          end;
          SetLength(yData2, 0);
        end;
        SetLength(yArray, Length(yArray) - 1);
      end;
      SetLength(yArray10, Length(yArray10) - 1);
    end;
    SetLength(yArray9, 0);
    while Length(yArray8) > 0 do
    begin
      SetLength(yArray8[Length(yArray8) - 1].yRef, 0);
      SetLength(yArray8, Length(yArray8) - 1);
    end;
    SetLength(yArray7, 0);
    SetLength(yArray6, 0);
    SetLength(yArray5, 0);
    SetLength(yArray4c, 0);
    SetLength(yArray4b, 0);
    while Length(yScriptList) > 0 do
    begin
      yScriptList[Length(yScriptList) - 1] := '';
      SetLength(yScriptList, Length(yScriptList) - 1);
    end;
    SetLength(yArray4, 0);
    SetLength(yArray3, 0);
    SetLength(yArray2, 0);
    ...
    SetLength(tUnknown, 0);
    SetLength(yQueuedUnbinds, 0);
    while Length(ySuspendedStacks2) > 0 do
      with ySuspendedStacks2[Length(ySuspendedStacks2) - 1]^ do
      begin
        SetLength(tMessageData.yUnknown, 0);
        Dispose(ySuspendedStacks2[Length(ySuspendedStacks2) - 1]);
        SetLength(ySuspendedStacks2, Length(ySuspendedStacks2) - 1);
      end;
    while Length(ySuspendedStacks1) > 0 do
      with ySuspendedStacks1[Length(ySuspendedStacks1) - 1]^ do
      begin
        SetLength(tMessageData.yUnknown, 0);
        Dispose(ySuspendedStacks1[Length(ySuspendedStacks1) - 1]);
        SetLength(ySuspendedStacks1, Length(ySuspendedStacks1) - 1);
      end;
    while Length(yFunctionMessages) > 0 do
      with yFunctionMessages[Length(yFunctionMessages) - 1]^ do
      begin
        SetLength(tMessageData.yUnknown, 0);
        Dispose(yFunctionMessages[Length(yFunctionMessages) - 1]);
        SetLength(yFunctionMessages, Length(yFunctionMessages) - 1);
      end;
    while Length(yActiveData) > 0 do
    begin
      ClearActiveData(yActiveData[Length(yActiveData) - 1]^);
      Dispose(yActiveData[Length(yActiveData) - 1]);
      SetLength(yActiveData, Length(yActiveData) - 1);
    end;
    while Length(yArrayData) > 0 do
      with yArrayData[Length(yArrayData) - 1]^ do
      begin
        while Length(yMembers) > 0 do
        begin
          Dispose(yMembers[Length(yMembers) - 1]);
          SetLength(yMembers, Length(yMembers) - 1);
        end;
        Dispose(yArrayData[Length(yArrayData) - 1]);
        SetLength(yArrayData, Length(yArrayData) - 1);
      end;
    while Length(yReferenceData) > 0 do
      with yReferenceData[Length(yReferenceData) - 1]^ do
      begin
        while Length(yMembers) > 0 do
        begin
          Dispose(yMembers[Length(yMembers) - 1]);
        ...
        Dispose(yReferenceData[Length(yReferenceData) - 1]);
        SetLength(yReferenceData, Length(yReferenceData) - 1);
      end;
    while Length(yInstanceData) > 0 do
      with yInstanceData[Length(yInstanceData) - 1]^ do
      begin
        while Length(yMembers) > 0 do
        begin
          Dispose(yMembers[Length(yMembers) - 1]);
        ...
        Dispose(yInstanceData[Length(yInstanceData) - 1]);
        SetLength(yInstanceData, Length(yInstanceData) - 1);
      end;
    while Length(yActives) > 0 do
    begin
      Dispose(yActives[Length(yActives) - 1]);
      SetLength(yActives, Length(yActives) - 1);
    end;
    while Length(yArrays) > 0 do
    begin
      Dispose(yArrays[Length(yArrays) - 1]);
      SetLength(yArrays, Length(yArrays) - 1);
    end;
    while Length(yReferences) > 0 do
    begin
      Dispose(yReferences[Length(yReferences) - 1]);
      SetLength(yReferences, Length(yReferences) - 1);
    end;
    while Length(yInstances) > 0 do
    begin
      Dispose(yInstances[Length(yInstances) - 1]);
      SetLength(yInstances, Length(yInstances) - 1);
    end;
    while Length(yScripts) > 0 do
      with yScripts[Length(yScripts) - 1]^ do
      begin
        while Length(yParameters) > 0 do
        begin
          Dispose(yParameters[Length(yParameters) - 1]);
          SetLength(yParameters, Length(yParameters) - 1);
        end;
        Dispose(yScripts[Length(yScripts) - 1]);
        SetLength(yScripts, Length(yScripts) - 1);
      end;
    while Length(yStrings) > 0 do
    begin
      yStrings[Length(yStrings) - 1]^.sName := '';
      Dispose(yStrings[Length(yStrings) - 1]);
      SetLength(yStrings, Length(yStrings) - 1);
    end;
  end;
  ZeroMemory(@Papyrus, SizeOf(Papyrus));
end;

function GetName(iName: integer; var sPapytus: TPapyrus): string;
begin
  if (iName > 0) and ((iName - 1) < Length(sPapytus.yStrings)) then
    Result := sPapytus.yStrings[iName - 1]^.sName
  else
    Result := '';
end;
...
begin
  case iType of
    ...
    4: Result := 'float';
    5: Result := 'boolean';
    11: Result := 'reference array';
    12: Result := 'string array';
    13: Result := 'integer array';
    14: Result := 'float array';
    15: Result := 'boolean array';
  else
    Result := 'unknow';
  end;
end;
