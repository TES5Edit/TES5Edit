...
type
  ...
  private
    ...
    procedure CheckGlobal(var Table1, Table2: array of PGlobalData);
  public
    ...
    if aAscend then
      case aIndex of
        ...
        5: while aDiffs[iL].Size2 < Act.Size2 do
             inc(iL);
        6: while aDiffs[iL].Type1 < Act.Type1 do
             inc(iL);
        7: while aDiffs[iL].Type2 < Act.Type2 do
             inc(iL);
        8: while aDiffs[iL].Flags1 < Act.Flags1 do
             inc(iL);
        9: while aDiffs[iL].Flags2 < Act.Flags2 do
             inc(iL);
    ...
    if aAscend then
      case aIndex of
        ...
        5: while aDiffs[iH].Size2 > Act.Size2 do
             dec(iH);
        6: while aDiffs[iH].Type1 > Act.Type1 do
             dec(iH);
        7: while aDiffs[iH].Type2 > Act.Type2 do
             dec(iH);
        8: while aDiffs[iH].Flags1 > Act.Flags1 do
             dec(iH);
        9: while aDiffs[iH].Flags2 > Act.Flags2 do
             dec(iH);
...
begin
  if Item.Index < Length(Diffs) then
      ...
      begin
        ...
        if DiffState <> dsOnly2nd then
          SubItems.Add(IntToStr(Size1))
        else
          SubItems.Add('');
        if DiffState <> dsOnly1st then
...
begin
  ...
  begin
    ...
    mmDiff.Visible := (lvDiffs.Selected.Data <> nil) and ((lvDiffs.Selected.Index) < Length(Diffs)) and (Diffs[lvDiffs.Selected.Index].DiffState = dsContent);
    ...
    if mmDiff.Visible then
      CompareData(Diffs[lvDiffs.Selected.Index].Data1, Diffs[lvDiffs.Selected.Index].Data2);
  end;
end;

procedure TfCompare.CheckGlobal(var Table1, Table2: array of PGlobalData);
var
  i, j: integer;
  bFound: boolean;
begin
...
begin
  ...
  try
    ...
    while (i < Length(Forms1)) and (j < Length(Forms2)) do
    begin
      case RefIDCompare(Forms1[i].Form.formID, Forms2[j].Form.formID) of
        ...
        1: while (RefIDCompare(Forms1[i].Form.formID, Forms2[j].Form.formID) > 0) and (j < Length(Forms2)) do
           begin
             SetLength(Diffs, Length(Diffs) + 1);
             with Diffs[Length(Diffs) - 1] do
             begin
             ...
             Inc(j);
             if Length(Forms2) > Length(Forms1) then
               Inc(p);
           end;
        -1: while (RefIDCompare(Forms1[i].Form.formID, Forms2[j].Form.formID) < 0) and (i < Length(Forms1)) do
            begin
              SetLength(Diffs, Length(Diffs) + 1);
              with Diffs[Length(Diffs) - 1] do
              begin
              ...
              Inc(i);
              if Length(Forms1) > Length(Forms2) then
                Inc(p);
            end;
        else
