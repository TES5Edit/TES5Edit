  ...
  case ltLog of
    ltTES4RuntimeScriptProfiler: begin
      if Node.Parent = Sender.RootNode then
        ...
        memoText.Lines.Add(Format('; RuntimeScriptProfiler - 最大执行时间：%.4n ms', [Data.PEntry.Value3]));
...
begin
  ...
  if (fid shr 24 <> $FF) and not Assigned(elem) then begin
    if FormIDErrors = 0 then
      memoText.Lines.Add('未知的 FormID [' + s + ']，是否修改过排序？所有未知的 FormID 都会被忽略掉的哦。');
    Inc(FormIDErrors);
    Exit;
  end;
...
begin
  ...
  try
    memoText.Lines.Add('正在处理日志...');
    if ProcessLog(edLogFile.Text) then begin
      BuildPluginsList;
      vstForms.NodeDataSize := SizeOf(TTreeData);
      vstForms.RootNodeCount := Length(LogPlugins);
      if FormIDErrors > 0 then
        memoText.Lines.Add(Format('已忽略 %d 个未知 FormIDs 。', [FormIDErrors]));
      memoText.Lines.Add('已完成。');
      memoText.Lines.Add('提示：双击 FormID 可以查看该 ID 的详细信息。');
    end else begin
      SetLength(LogPlugins, 0);
      SetLength(LogEntries, 0);
    end;
...
begin
  case ltLog of
    ltTES4RuntimeScriptProfiler: begin
      ...
      vstForms.Header.Columns[2].Text := '执行帧数';
      vstForms.Header.Columns[2].Width := 100;
      vstForms.Header.Columns[3].Text := '全部时间, ms';
    ...
    ltTES5Papyrus: begin
      ...
      vstForms.Header.Columns[2].Width := 100;
      vstForms.Header.Columns[2].Text := '错误';
      vstForms.Header.Columns[2].Width := 100;
      vstForms.Header.Columns[3].Text := '警告';
      memoText.Lines.Add('如何开启日志');
      memoText.Lines.Add('http://www.creationkit.com/FAQ:_My_Script_Doesn''t_Work!');
      memoText.Lines.Add('Papyrus 错误介绍');
