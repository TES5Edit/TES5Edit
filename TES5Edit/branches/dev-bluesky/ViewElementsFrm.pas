...
begin
  ...
  try
    ...
    on E: Exception do
      MessageBox(0, PChar('无法执行命令行'#13 + aExe + ' ' + aParams + #13'错误：' + E.Message), '错误', 0);
  end;
end;
...
begin
  ...
  if InputQuery('比较程序', '命令行 (%1 与 %2 是缓存文本文件)', s) then begin
    CompareCmdLine := s;
    ...
    for i := 0 to Pred(Edits.Count) do
      with TwbEdit(Edits[i]) do
        if eMemo.Modified then try
            ...
            on E: Exception do
              ShowMessage(Format('任务错误：%s'#13#10'文件：%s'#13#10'元素：%s',
