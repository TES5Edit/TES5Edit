...
function TColorize.SaveToFile(const AFileName: string; Verbose: boolean ): boolean;
  ...
  if not DirectoryExists( path ) then
    if Verbose and ( MessageDlg('创建目录 ' + path + ' ？', mtWarning, [mbYes, mbNo], 0) <> mrYes ) then
      exit
    else
  ...
  if FileExists( FFileName ) then
    begin
    if Verbose and ( MessageDlg('您确定要替换 ' + FFileName + ' ？', mtWarning, [mbYes, mbNo], 0) <> mrYes ) then
      exit
    else
