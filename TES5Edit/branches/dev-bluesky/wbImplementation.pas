...
begin
  ...
  flProgress('正在添加 Master "' + t + '"');
  _File := wbFile(s + t);
  if wbRequireLoadOrder and (_File.LoadOrder < 0) then
    raise Exception.Create('"' + GetFileName + '" 需要 Master  "' + aFileName + '" 加载在前面。');
  AddMaster(_File);
end;
...
begin
  if not IsElementEditable(nil) then
    raise Exception.Create('文件 "'+GetFileName+'" 无法编辑');
...
begin
  ...
  if not Supports(aElement, IwbGroupRecord, GroupRecord) then
    raise Exception.Create('仅群组记录可以添加到文件');
  if GroupRecord.GroupType <> 0 then
    raise Exception.Create('仅顶级群组记录可以添加到文件');
  Signature := TwbSignature(GroupRecord.GroupLabel);
  if not wbGroupOrder.Find(Signature, Dummy) then
    raise Exception.Create(Signature + '不是有效的群组标签');
  Result := GetGroupBySignature(Signature);
    ...
    if FileID >= Cardinal(GetMasterCount) then begin
      ...
      on E: Exception do
        if Assigned(wbProgressCallback) then
          wbProgressCallback('错误：<'+e.Message+'> 发生在判断 ' + aRecord.Name + '的 Master 记录');
    end;
  end;
  ...
  if Assigned(MasterFiles) then
    for i := 0 to Pred(MasterFiles.ElementCount) do begin
      ...
      if not Assigned(Rec) then
        raise Exception.CreateFmt('读取文件 "%s" 的 Master 列表时遇到非预期的错误', [flFileName]);
...
begin
  if Length(flInjectedRecords) > 0 then begin
    if FindInjectedID(aRecord.FormID, i) then begin
      if [fsIsHardcoded, fsIsCompareLoad] * flInjectedRecords[i]._File.FileStates = [] then begin
        if Assigned(wbProgressCallback) then
          wbProgressCallback('<警告：' + aRecord.Name + ' 要注入的对象 ' + GetFileName + ' 已经被来自于文件 '+flInjectedRecords[i]._File.FileName+' 的 '+flInjectedRecords[i].Name+' 注入了。>');
      end;
  ...
  if [fsIsHardcoded, fsIsCompareLoad] * aRecord._File.FileStates = [] then begin
    if Assigned(wbProgressCallback) then
      wbProgressCallback('<注意：' + aRecord.Name + ' 被注入到 ' + GetFileName + '>');
  end;
...
begin
  ...
  if NewFileID < 0 then
    raise Exception.Create('加载顺序 FileID ['+IntToHex64(aFileID, 2)+'] 无法映射到文件 "'+GetFileName+'" 的文件 FileID。');
  Result := NewFileID;
end;
...
begin
  ...
  if Length(cntElements) < 1 then
    raise Exception.Create('文件 '+GetFileName+' 丢失文件头。');

  if cntElements[0].ElementType <> etMainRecord then
    raise Exception.Create('文件 '+GetFileName+' 存在无效记录 '+cntElements[0].Name+' 位于文件头。');

  FileHeader := cntElements[0] as IwbMainRecord;
  if FileHeader.Signature <> HeaderSignature then
    raise Exception.Create('文件 '+GetFileName+' 存在使用无效签名的无效记录 '+cntElements[0].Name+' 位于文件头。');

  HEDR := FileHeader.RecordBySignature['HEDR'];
  if not Assigned(HEDR) then
    raise Exception.Create('文件 '+GetFileName+' 存在丢失子记录 HEDR 的文件头。');
...
begin
  ...
  if cntElements[0].ElementType <> etMainRecord then
    raise Exception.Create('文件 '+GetFileName+' 包含无效记录 '+cntElements[0].Name+' 位于文件头。');

  FileHeader := cntElements[0] as IwbMainRecord;
  if FileHeader.Signature <> HeaderSignature then
    raise Exception.Create('文件 '+GetFileName+' 包含含无效签名的无效记录 '+cntElements[0].Name+' 位于文件头。');
  ...
  for i := Succ(Low(cntElements)) to High(cntElements) do begin
    if not Supports(cntElements[i], IwbGroupRecord, GroupRecord) then
      raise Exception.Create('文件 '+GetFileName+' 包含无效的顶部等级记录： '+ cntElements[i].Name);
    if GroupRecord.GroupType <> 0 then
      raise Exception.Create('文件 '+GetFileName+' 包含无效的顶部等级群组类型 '+IntToStr(GroupRecord.GroupType)+' 出现在群组：'+ cntElements[i].Name);
    if GroupRecord.SortOrder < 0 then
      raise Exception.Create('文件 '+GetFileName+' 包含不存在已知排序规则的顶部等级群组：'+ cntElements[i].Name);
    if GroupRecord.SortOrder > High(Groups) then
      raise Exception.Create('文件 '+GetFileName+' 包含存在无效排序规则的顶部等级群组：'+ cntElements[i].Name);
    if Groups[GroupRecord.SortOrder] then
      raise Exception.Create('文件 '+GetFileName+' 包含重复顶级等级群组： '+ cntElements[i].Name);
  ...
  if RecordCount < 1 then
    raise Exception.Create('文件 '+GetFileName+' 出现无效的记录数目');
                  ...
                  if wbMasterUpdateFixPersistence and not Current.IsPersistent and not Current.IsMaster then begin
                    ...
                    if Assigned(Master) then begin
                      if Master.IsPersistent then begin
                        flProgress('正在设置 Persistent ：' + Current.Name);
...
begin
  ...
  if aRecord.FormID <> 0 then begin
    ...
    if FileID >= Cardinal(GetMasterCount) then begin
      ...
      on E: Exception do
        if Assigned(wbProgressCallback) then
          wbProgressCallback('错误：<'+e.Message+'> 发生于判断 ' + aRecord.Name +' 的 Master 记录。');
    end;
  end;
end;
...
begin
  ...
  if Header.Signature <> HeaderSignature then
    raise Exception.CreateFmt('预期的 Header 签名 TES4，已找到 %s 位于文件 "%s"', [String(Header.Signature), flFileName]);
    ...
    try
      ...
      for i := High(cntElements) downto Succ(Low(cntElements)) do begin
        if not Supports(cntElements[i], IwbGroupRecord, GroupRecord) then begin
          flProgress('错误：文件包含无效的顶部等级记录：'+ cntElements[i].Name);
          Continue;
        end;
        if GroupRecord.GroupType <> 0 then begin
          flProgress('错误：文件包含无效的顶部等级群组类型 '+IntToStr(GroupRecord.GroupType)+' 出现在群组：'+ cntElements[i].Name);
          Continue;
        end;
        if GroupRecord.SortOrder < 0 then begin
          flProgress('错误：文件包含不存在已知排序的顶部等级群组：'+ cntElements[i].Name);
          Continue;
        end;
        if GroupRecord.SortOrder > High(Groups) then begin
          flProgress('错误：文件包含存在无效排序的顶部等级群组：'+ cntElements[i].Name);
          Continue;
        end;
        if Assigned(Groups[GroupRecord.SortOrder]) then begin
          flProgress('警告：文件包含重复顶级等级群组：' + cntElements[i].Name);
          if wbBeginInternalEdit(True) then try
            ...
            flProgress('已整合 ' + IntToStr(j) + ' 个来自于重复群组 ' + cntElements[i].Name + ' 的记录');
            GroupRecord.Remove;
          finally
            wbEndInternalEdit;
          end;
...
begin
  ...
  if not wbEditAllowed then
    raise Exception.Create(GetName + ' 无法分配。');
...
begin
  ...
  if not Assigned(cntElements) or (aIndex>=Length(cntElements)) then begin // Using the wrong contained array at the time
    if wbMoreInfoForIndex and (DebugHook <> 0) and Assigned(wbProgressCallback) then
      wbProgressCallback('调试期：['+ IwbElement(Self).Path +'] 索引 ' + IntToStr(aIndex) + ' 超过最大值 '+
...
begin
  if aMainRecord.Signature <> GetSignature then
    if Assigned(wbProgressCallback) then
      wbProgressCallback(Format('警告：记录 %s （来源于文件 %s ）正被记录 %s （来源于文件 %s ）所覆盖。', [
...
begin
  ...
  if not Supports(GetContainer, IwbGroupRecord, Group1) then
    raise Exception.Create(GetName + ' 不存在于群组之中。');
  if not (Group1.GroupType in [8, 9, 10]) then
    raise Exception.Create(GetName + ' 不存在于 "Cell Persistent Childen", "Cell Temporary Children" 或者 "Cell Visible Distant Children" 群组类型之中。');
  if not Supports(Group1.Container, IwbGroupRecord, Group2) then
    raise Exception.Create(Group1.GetName + ' 不存在于群组之中。');
  if not (Group2.GroupType in [6]) then
    raise Exception.Create(Group1.GetName + ' 不存在于 "Cell Children" 群组类型之中。');
end;
  ...
  while (CurrentRecPos < Length(cntElements)) do begin
    ...
    if mrDef.AllowUnordered then begin
      ...
      if CurrentDefPos < 0 then begin
        if Assigned(wbProgressCallback) then
          wbProgressCallback('错误：记录 '+ String(GetSignature) + ' 包含非预期的（或者过期的）子记录 ' + String(CurrentRec.Signature) + ' ' + IntToHex(Int64(Cardinal(CurrentRec.Signature)), 8) );
        FoundError := True;
        Inc(CurrentRecPos);
        Continue;
      end;
      ...
      if (CurrentDefPos < mrDef.MemberCount) then begin
        ...
        if Assigned(wbProgressCallback) then
          wbProgressCallback('错误：记录 '+ String(GetSignature) + ' 包含非预期的（或者过期的）子记录 ' + String(CurrentRec.Signature) );
        FoundError := True;
        Inc(CurrentRecPos);
        Continue;
      end;
    ...
    case CurrentDef.DefType of
      ...
      raise Exception.CreateFmt('非预期的 def 类型出现在子记录 %s 位于 %s', [String(CurrentRec.Signature), String(GetSignature)]);
    end;
        ...
        for i := 0 to Pred(mrDef.MemberCount) do
          if i in RequiredRecords then begin
            if wbMoreInfoForRequired  then
              wbProgressCallback(' ['+IntToHex64(mrFixedFormID, 8)+'] 正在添加丢失记录： ' + mrDef.Members[i].GetName);
            Assign(i, nil, False);
          end;
      finally
        wbEndInternalEdit;
...
begin
  ...
  try
    ...
    on E: Exception do begin
      if Assigned(wbProgressCallback) then
        wbProgressCallback('获取 "' + GetName + '" 方位出错：' + E.Message);
      Result := False;
      Exit;
    end;
  end;
...
begin
  ...
  if not Assigned(mrDef) then begin
    if wbRecordDefMap.Find(PwbSignature(dcBasePtr)^, i) then
      ...
      if Assigned(wbProgressCallback) then
        wbProgressCallback('错误：未知的记录类型 '+ String(PwbSignature(dcBasePtr)^));
    end;
  end;
end;
    ...
    case GroupRecord.GroupType of
      0: begin {top level}
        if TwbSignature(GroupRecord.GroupLabel) <> GetSignature then
          raise Exception.Create('记录 ' + GetName + ' 不能出现在 ' + GroupRecord.Name);
      end;
        ...
        case GroupRecord.GroupType of
          8:begin
            if not mrStruct.mrsFlags.IsPersistent then
              raise Exception.Create('记录 ' + GetName + ' 需要将 Persistent 标志定义在 ' + GroupRecord.Name);
          end;
          10: begin
            if not mrStruct.mrsFlags.IsVisibleWhenDistant then
              raise Exception.Create('记录 ' + GetName + ' 需要将远景可见标志定义在 ' + GroupRecord.Name);
            if mrStruct.mrsFlags.IsPersistent then
              raise Exception.Create('记录 ' + GetName + ' 不能将 Persistent 标志定义在 ' + GroupRecord.Name);
          end;
        end;
      end;
      9: begin {Temporary}
        ...
        if mrStruct.mrsFlags.IsVisibleWhenDistant and not wbVWDInTemporary then
          raise Exception.Create('记录 ' + GetName + ' 不能将远景可见标志定义在 ' + GroupRecord.Name);
      end;
    end;
  end;
  ...
  if Assigned(aGroup) then begin
    if not (not Assigned(mrGroup) or (mrGroup.Equals(aGroup))) then begin
      if not (not Assigned(mrGroup) or (mrGroup.Equals(aGroup))) then
        Assert(not Assigned(mrGroup) or (mrGroup.Equals(aGroup)), '发现额外的 ' + mrGroup.Name + ' 出现在 ' + Self.GetName);
    end;
    if Assigned(eContainer) then
...
begin
  ...
  if not Supports(OldTypeGroup.Container, IwbGroupRecord, OldChildGroup) then
    raise Exception.Create(OldTypeGroup.GetName + ' 不存在于群组之中。');
  if not (OldChildGroup.GroupType in [6]) then
    raise Exception.Create(OldTypeGroup.GetName + ' 不存在于 "Cell Children" 群组类型之中。');
  ...
  if not Assigned(OldCell) then
    raise Exception.Create(OldChildGroup.GetName + ' 无法找到对应的 CELL 记录');
  if not OldCell.ElementExists['DATA'] then
    raise Exception.Create(OldCell.GetName + ' 丢失对应的 DATA 记录');
  ...
  if IsExterior then begin
    if not Supports(OldCell.Container, IwbGroupRecord, OldCellOwnerGroup) then
      raise Exception.Create(OldCell.GetName + ' 不存在于群组之中。');
    if not (OldCellOwnerGroup.GroupType in [1, 5]) then
      raise Exception.Create(OldCell.GetName + ' 不存在于 "World Childen" 或者 "Exterior Cell Sub-Block" 群组类型之中。');
    if (CorrectGroupType = 8) then begin
      if OldCellOwnerGroup.GroupType <> 1 then begin
        ...
        if not Supports(OldCellOwnerGroup.Container, IwbGroupRecord, TempGroup) then
          raise Exception.Create(OldCellOwnerGroup.GetName + ' 不存在于群组之中。');
        if not (TempGroup.GroupType in [4]) then
          raise Exception.Create(OldCellOwnerGroup.GetName + ' 不存在于 "Exterior Cell Block" 群组类型之中。');

        if not Supports(TempGroup.Container, IwbGroupRecord, NewCellOwnerGroup) then
          raise Exception.Create(TempGroup.GetName + ' 不存在于群组之中。');
        if not (NewCellOwnerGroup.GroupType in [1]) then
          raise Exception.Create(TempGroup.GetName + ' 不存在于 "World Childen" 群组类型之中。');
        ...
        if not Assigned(NewCell) then begin
          ...
          if not Assigned(Worldspace) then
            raise Exception.Create(NewCellOwnerGroup.GetName + ' 无法找到对应的 WRLD 记录');
          Worldspace := Worldspace.MasterOrSelf;
          TempGroup := Worldspace.ChildGroup;
          if not Assigned(TempGroup) then
            raise Exception.Create(Worldspace.GetName + ' 无法找到对应的 child group');
        ...
        if not Assigned(NewCell) then
          raise Exception.Create('无法判断 persistent exterior references 的 CELL');
      ...
      if OldCellOwnerGroup.GroupType <> 5 then begin
        ...
        if not GetPosition(Position) then
          raise Exception.Create('无法判断 ' + GetName + ' 的方位');
        ...
        if not Assigned(NewCell) then begin
          ...
          if not Assigned(Worldspace) then
            raise Exception.Create(OldCellOwnerGroup.GetName + ' 无法找到对应的 WRLD 记录');
          Worldspace := Worldspace.MasterOrSelf;
  ...
  if not Assigned(NewChildGroup) then
    raise Exception.Create('无法判断新型 CELL child group');
...
begin
  ...
  if not (SubBlockGroup.GroupType in [3]) then begin
    if (SubBlockGroup.GroupType in [0]) and (TwbSignature(SubBlockGroup.GroupLabel) = 'CELL') then begin
      ...
      raise Exception.Create(GetName + ' 不存在于 "Interior Cell Sub-Block" 群组类型之中。');
  end else begin
    if not Supports(SubBlockGroup.Container, IwbGroupRecord, BlockGroup) then
      raise Exception.Create(SubBlockGroup.GetName + ' 不存在于群组之中。');
    if not (BlockGroup.GroupType in [2]) then
      raise Exception.Create(GetName + ' 不存在于 "Interior Cell Block" 群组类型之中。');

    if not Supports(BlockGroup.Container, IwbGroupRecord, TopGroup) then
      raise Exception.Create(TopGroup.GetName + ' 不存在于群组之中。');
    if not (TopGroup.GroupType in [0]) or (TwbSignature(TopGroup.GroupLabel) <> 'CELL') then
      raise Exception.Create(GetName + ' 不存在于 "Top CELL" 群组类型之中。');
  end;
...
begin
  ...
  if Assigned(dcDataEndPtr) and Assigned(BasePtr) and (BasePtr <> dcDataEndPtr) then begin
    ...
    if HasUnusedData then begin
      ...
      {$IFDEF DBGSUBREC}
      if Assigned(wbProgressCallback) then
        wbProgressCallback('<警告：未使用的数据出现在：' + GetFullPath + '>');
      {$ENDIF}
    end;
  end;
...
begin
  ...
  if Assigned(MainRecord) then begin
    ...
    if MainRecord.Signature <> Signature then
      raise Exception.Create('已存在记录 '+MainRecord.Name+' 含不同签名。');
  end;
  ...
  case grStruct.grsGroupType of
    0: begin
      if TwbSignature(grStruct.grsLabel) = 'DIAL' then begin
        if Supports(aElement, IwbGroupRecord, GroupRecord) then begin
          if GroupRecord.GroupType <> 7 then
            raise Exception.Create('无法添加 '+GroupRecord.Name+' 到含签名 ' + TwbSignature(grStruct.grsLabel) + ' 的顶级等级群组。');
          MainRecord := GroupRecord.ChildrenOf;
          if not Assigned(MainRecord) then
            raise Exception.Create('无法找到 '+ GroupRecord.Name + ' 的记录。');
      ...
      if not Supports(aElement, IwbMainRecord, MainRecord) then
        raise Exception.Create('仅主要记录可以添加到顶级等级群组。');
      if MainRecord.Signature <> TwbSignature(grStruct.grsLabel) then
        raise Exception.Create('无法添加含签名 '+MainRecord.Signature+' 的主要记录到含签名 ' + TwbSignature(grStruct.grsLabel) + ' 的顶级等级群组。');
    ...
    1: begin
      ...
      if not Supports(aElement, IwbMainRecord, MainRecord) then
        raise Exception.Create('仅主要记录可以添加到 ' + GetName);
      if (MainRecord.Signature <> 'CELL') and (MainRecord.Signature <> 'ROAD') then
        raise Exception.Create('无法添加含签名 '+MainRecord.Signature+' 的主要记录到 ' + GetName);

      if aAsNew then
        raise Exception.Create('无法复制记录 '+MainRecord.Name+' 为新记录。')
      else begin
        FormID := GetFile.LoadOrderFormIDtoFileFormID(MainRecord.LoadOrderFormID);
        for i := Low(cntElements) to High(cntElements) do
          if Supports(cntElements[i], IwbMainRecord, MainRecord2) then
    ...
    2, 4: begin
      if Supports(aElement, IwbGroupRecord, GroupRecord) then begin
        if GroupRecord.GroupType <> grStruct.grsGroupType + 1 then
          raise Exception.Create('无法添加 '+GroupRecord.Name+' 到 ' + GetName);
        for i := 0 to Pred(GetElementCount) do
          if Supports(GetElement(i), IwbGroupRecord, GroupRecord2) then begin
            if (GroupRecord2.GroupType = GroupRecord.GroupType) and (GroupRecord2.GroupLabel = GroupRecord.GroupLabel) then begin
              Result := GroupRecord2;
      ...
      raise Exception.Create('无法添加 ' + aElement.Name + ' 到 ' + GetName);
    end;
    3, 5: begin
        if Supports(aElement, IwbGroupRecord, GroupRecord) then begin
          if GroupRecord.GroupType <> 6 then
    ...
    raise Exception.Create(ClassName + '.AddIfMissing 对 GroupType ' + IntToStr(grStruct.grsGroupType) + ' 无法处理。');
  end;
end;
  ...
  if GetGroupType in [1, 6, 7] then begin
    ...
    if Assigned(Rec) then begin
      if Rec._File.Equals(GetFile) then
        ...
        if Assigned(wbProgressCallback) then
          wbProgressCallback('<警告：含群组 ' + GetName + ' 的文件 ' + GetFile.Name + ' 丢失 ' + Rec.Name + ' 的覆盖记录。');
      end;
    end;
  end;
end;
...
begin
  ...
  case grStruct.grsGroupType of
    ...
    Result := Result + ' 未知类型： ' + IntToStr(grStruct.grsGroupType);
    Exit;
  end;
...
begin
  raise Exception.Create(ClassName + '.AddIfMissing 未处理。');
end;
    ...
    if sl.Count > 0 then begin
      ...
      for i := 0 to Pred(sl.Count) do
        if IwbFile(Pointer(sl.Objects[i])).LoadOrder >= aTargetFile.LoadOrder then
          raise Exception.Create('需要的 Master 文件 "' + sl[i] + '" 无法添加到 "' + aTargetFile.FileName + '" ，因为它有更高的加载顺序。');
...
begin
  Assert(False, '无法应用 SetDataSize 在 ' + ClassName);
end;
...
begin
  ...
  while (aPos < aContainer.ElementCount) and
    ...
    case ElementDef.DefType of
      ...
      raise Exception.CreateFmt('非预期的子记录 %s def 类型出现在', [String(SubRecord.Signature)]);
    end;
  ...
  if aPos = Low(Integer) then begin
    ...
    while (aPos < aContainer.ElementCount) and (CurrentDefPos < srcDef.MemberCount) do begin
      ...
      case CurrentDef.DefType of
        ...
        raise Exception.CreateFmt('非预期的子记录 %s def 类型', [String(CurrentRec.Signature)]);
      end;
...
begin
  ...
  if FilesMap.Find(FileName, i) then
    raise Exception.Create(FileName + ' 已存在')
  else begin
