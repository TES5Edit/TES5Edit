...
begin
  ...
  if InputQuery('新 FormID', '请以16进制输入新的 FormID ，例：0404CC43。要求前两位为插件加载顺序。', s) then try
    Result := StrToInt64('$' + s);
  except
    on E: Exception do
...
begin
  ...
  try
    ...
    if sl.Count > 0 then begin
      ...
      Result := MessageDlg('如果要继续，以下文件必须添加到 "' +
        aTargetFile.FileName + '" 并作为 Master ：'#13#13 + sl.Text +
    ...
    with TfrmFileSelect.Create(Self) do try
      ...
      if not Multiple then begin
        ...
        if not (AsNew or AsWrapper) then begin
          ...
          repeat
            if AsWrapper then begin
              if not InputQuery('EditorID', '请输入封装副本的 EditorID', EditorID) then
                Exit;
            end
            else begin
              if not InputQuery('EditorID', '请修改 EditorID', EditorID) then
                Exit;
            end;
            if EditorID = '' then
              Break;
            if not SameText(EditorID, MainRecord.EditorID) then
              Break;
            if AsWrapper then
              ShowMessage('您需要给封装副本定义不同的 EditorID。')
            else if MessageDlg('您确定不要修改 EditorID 吗？' +
              'EditorID 冲突会导致插件在 CS 加载中出现错误信息。',
        ...
        if AsNew or AsWrapper then
          repeat
            if not InputQuery('EditorID 前缀', '请输入应该从 EditorID 中移除的前缀', EditorIDPrefixRemove) then
              Exit;
            if not InputQuery('EditorID 前缀', '请输入应该添加到 EditorID 的前缀', EditorIDPrefix) then
              Exit;
            if not InputQuery('EditorID 后缀', '请输入应该添加到 EditorID 的后缀', EditorIDSuffix) then
              Exit;
...
begin
  ...
  if InputQuery('新 FormID', '请以十六进制数输入新的 FormID ，例：0404CC43。前两位数必须符合插件的加载顺序。', s) then try
    NewFormID := StrToInt64('$' + s);
    if NewFormID = 0 then
      raise Exception.Create('00000000 不是有效的 FormID');
    if NewFormID = $14 then
      raise Exception.Create('00000014 不是有效的 FormID');

    OldFormID := MainRecord.LoadOrderFormID;
    if NewFormID = OldFormID then begin
      ShowMessage('前后两次的 FormID 是一样的');
      Exit;
    end;
...
begin
  ...
  AddMessage(wbAppName + 'Edit ' + VersionString + ' 正在启动进程 ' + FormatDateTime('yyyy-mm-dd hh:nn:ss', Now));
...
begin
  ...
  try
    ...
    PostAddMessage('[Apply Script done] ' + ' 已处理记录：' + IntToStr(Count) +
      ', 持续时间：' + FormatDateTime('nn:ss', Now - wbStartTime));
      ...
      try
        ...
        for i := 1 to Pred(CSV.Count) do begin //ignore first line
          ...
          if (j < 0) or (j > $FFFFFF) then begin
            AddMessage('跳过行'+IntToStr(i+1)+'：旧 FormID "'+s+'" 不在有效范围内。');
            Continue;
          end;
          ...
          if not Assigned(OldRecord) then begin
            AddMessage('跳过行'+IntToStr(i+1)+'：FormID 为 "'+s+'" 的旧记录在旧 Master "'+OldMaster.FileName+'" 中未找到。');
            Continue;
          end;
          ...
          j := StrToIntDef(s, -1);
          if (j < 0) or (j > $FFFFFF) then begin
            AddMessage('跳过行'+IntToStr(i+1)+'：新 FormID "'+s+'" 不在有效范围内。');
            Continue;
          end;
          ...
          if not Assigned(NewRecord) then begin
            AddMessage('跳过行'+IntToStr(i+1)+'：FormID 为 "'+s+'" 的旧记录在新 Master "'+NewMaster.FileName+'" 中未找到。');
            Continue;
          end;
          ...
          if not SameText(OldRecord.EditorID, s) then
            AddMessage('警告行'+IntToStr(i+1)+'：旧记录不存在预测的 EditorID ("'+OldRecord.EditorID+'" vs. "'+s+'")"。');

          s := Trim(Line[4]);
          if not SameText(NewRecord.EditorID, s) then
            AddMessage('警告行'+IntToStr(i+1)+'：新记录不存在预测的 EditorID ("'+NewRecord.EditorID+'" vs. "'+s+'")"。');
    ...
    if AddRequiredMasters(NewMasters, _File) then
      for i := 0 to Pred(l) do with ReplaceList[i] do begin
        ...
        if Assigned(RefRecord) and _File.Equals(RefRecord._File) then begin
          AddMessage('正在修改 FormID ['+IntToHex64(RefRecord.LoadOrderFormID, 8)+'] 为 ['+IntToHex(rlNewRecord.LoadOrderFormID, 8)+']');
    ...
    end;
{
  ...
  for j := Low(Nodes) to High(Nodes) do begin
    ...
    if not Assigned(_File) then begin
      ...
      if InputQuery('新 FormID', '请以十六进制数输入新的 FormID ，例：0404CC43。前两位数必须符合插件的加载顺序。', s) then begin

        if s = '' then begin
          s := IntToHex64(MainRecord._File.FileFormIDtoLoadOrderFormID(MainRecord._File.NewFormID), 8);
          if not InputQuery('已生成新 FormID', '请插件新生成的 FormID 。前两位数必须符合插件的加载顺序。', s) then
            Exit;
        end;
        NewFormID := StrToInt64('$' + s);
        if NewFormID = 0 then
    ...
    AddMessage('正在修改 FormID ['+IntToHex64(OldFormID, 8)+'] in file "'+MainRecord._File.FileName+'" to ['+IntToHex(NewFormID, 8)+']');
      ...
      if not (AsNew or AsWrapper) then begin
        ...
        repeat
          ...
          if AsWrapper then begin
            ...
            if MessageDlg('您确定不要修改 EditorID 吗？' +
              'EditorID 冲突会导致插件在 CS 加载中出现错误信息。',
              mtWarning, [mbYes, mbNo], 0, mbNo) = mrYes then
              Break;
          end;
...
begin
  ...
  repeat
    if s <> '' then
      ShowMessage('"'+s+'" 不是有效的 start FormID.');

    if not InputQuery('起始于...', '请以十六进制输入新 FormID 的起点，例：200000。只需要输入后面6位数。', s) then
      Exit;
  ...
  try
    j := 0;
    Caption := '[正在修改 FormIDs] 已处理记录：' + IntToStr(0) +
      ' 持续时间：' + FormatDateTime('nn:ss', Now - wbStartTime);
    for k := High(MainRecords) downto Low(MainRecords) do begin
      ...
      AddMessage('正在修改文件 '+MainRecord._File.FileName+' 的 FormID ['+IntToHex64(OldFormID, 8)+'] 为 ['+IntToHex(NewFormID, 8)+']');
      ...
      if StartTick + 500 < GetTickCount then begin
        Caption := '[正在修改 FormIDs] 已处理记录：' + IntToStr(High(MainRecords) - k) +
          ' 持续时间：' + FormatDateTime('nn:ss', Now - wbStartTime);
        Application.ProcessMessages;
        StartTick := GetTickCount;
        if ForceTerminate then
...
begin
  with TfrmFileSelect.Create(nil) do try
    ...
    if Counter <= 0 then begin
      if not aSilent then
        ShowMessage('一共有 ' + IntToStr(Length(ReferencedBy)) + ' 个记录引用了 FormID ' + IntToHex64(OldFormID, 8) + ' 但没有一个是位于可编辑文件内。');
      Exit;
    end;
    ...
    if CheckListBox1.Count > 0 then begin
      ...
      AddMessage(IntToStr(Counter) + '/'+IntToStr(Length(ReferencedBy))+' 引用 FormID ['+IntToHex64(OldFormID, 8)+'] 的记录已更新到 ['+IntToHex64(NewFormID, 8)+']');
