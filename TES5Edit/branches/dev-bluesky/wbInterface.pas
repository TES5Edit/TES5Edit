...
function wbString(const aName      : string = '未知';
                        aSize      : Integer = 0;
                        aPriority  : TwbConflictPriority = cpNormal;
                        aRequired  : Boolean = False;
                        aDontShow  : TwbDontShowCallback = nil;
...
function wbByteArray(const aName      : string = '未知';
                           aSize      : Int64 = 0;
                           aPriority  : TwbConflictPriority = cpNormal;
                           aRequired  : Boolean = False;
                           aDontShow  : TwbDontShowCallback = nil)
...
function wbFloat(const aName       : string = '未知';
                       aPriority   : TwbConflictPriority = cpNormal;
                       aRequired   : Boolean = False;
                       aScale      : Extended = 1.0;
                       aDigits     : Integer = -1;
...
begin
  ...
  if Length(t) >= 4 then
    ...
    raise Exception.Create('"'+t+'" 不是有效的签名');
end;
...
//    aAllowUnordered := True;
  ...
  if wbRecordDefMap.IndexOf(aSignature) >= 0 then
    raise Exception.CreateFmt('签名 %s 重复定义', [String(aSignature)]);
...
function wbUnknown(const aSignature : TwbSignature;
                         ...
                         aDontShow  : TwbDontShowCallback = nil)
                                    : IwbSubRecordDef;
begin
  Result := wbByteArray(aSignature, '未知', 0, aPriority, aRequired, False, aDontShow);
end;
...
function wbUnknown(aPriority : TwbConflictPriority = cpNormal;
                   ...
                   aDontShow : TwbDontShowCallback = nil)
                             : IwbByteArrayDef;
begin
  Result := wbByteArray('未知', 0, aPriority, aRequired, aDontShow);
end;
...
begin
  ...
  if wbReportUnused then
    if not defUsed then
      WriteLn('未使用：', wbDefsToPath(aParents), wbDefToName(Self));

  if wbReportRequired and defPossiblyRequired then
    if defNotRequired = defRequired then
      if defNotRequired then
        WriteLn('不需要：', wbDefsToPath(aParents), wbDefToName(Self))
      else
        WriteLn('需要：', wbDefsToPath(aParents), wbDefToName(Self));

  if wbReportUnknown then
    if Assigned(UnknownValues) then begin
      WriteLn('未知域：', wbDefsToPath(aParents), wbDefToName(Self), ' (', UnknownValues.Count ,')');
      ...
      if IsUnknown then
        WriteLn('未知域：', wbDefsToPath(aParents), wbDefToName(Self));
  ...
  if not IsUnknown then
    if not IsUnknownChecked then begin
      ...
      if Supports(defParent, IwbNamedDef, NamedDef) then
        if Pos('未知', LowerCase(NamedDef.Name)) > 0 then
          IsUnknown := True;
    end;
...
begin
  ...
  if Pos('未知', LowerCase(aName)) > 0 then
    IsUnknown := True;
end;
...
begin
  ...
  if not IsUnknown and (noName = '') and Supports(defParent, IwbNamedDef, Parent) then
    IsUnknown := Pos('未知', LowerCase(Parent.Name)) > 0;
end;
...
begin
  ...
  if wbReportUnusedData then
    if srHasUnusedData then
      WriteLn('含未使用记录：', wbDefsToPath(aParents), wbDefToName(Self));
...
begin
  ...
  if Len < ExpectedLen[inType] then begin
    if wbCheckExpectedBytes then
      Result := Format('预测 %d 字节的数据，找到 %d', [ExpectedLen[inType] , Len])
  end else begin
    case inType of
      itS8:  Value := PShortInt(aBasePtr)^;
      itU16: Value := PWord(aBasePtr)^;
...
begin
  ...
  if Len < ExpectedLen[inType] then begin
    if wbCheckExpectedBytes then
      Result := Format('<错误：预测 %d 字节的数据，找到 %d>', [ExpectedLen[inType] , Len])
  end else begin
    case inType of
      itS8:  Value := PShortInt(aBasePtr)^;
      itU16: Value := PWord(aBasePtr)^;
    ...
    if Len > ExpectedLen[inType] then begin
      if wbCheckExpectedBytes then
        Result := Result + Format(' <警告：预测 %d 字节的数据，找到 %d>', [ExpectedLen[inType] , Len])
    end;
  end;
...
begin
  ...
  if Assigned(aElement) then begin
    ...
    if not Assigned(Container) and (not SameText(noName, '未使用') or not wbHideUnused) then begin
      Result := High(Integer);
      Exit;
    end;
  end;
        ...
        while (Count > Index) and (Cardinal(EndPtr) < Cardinal(aEndPtr)) do begin
          ...
          if not Assigned(Element) then begin
            if wbMoreInfoForIndex and (DebugHook <> 0) and Assigned(wbProgressCallback) then
              wbProgressCallback('调试：['+ Container.Path +'] ' + IntToStr(Count) + ' 的索引 ' + IntToStr(Index) + ' 大于最大值 '+
...
begin
  ...
  if (Cardinal(aBasePtr) > Cardinal(aEndPtr)) then // if aBasePtr >= aEndPtr then no allocation (or error)
    wbProgressCallback('找到负数体积的结构！'+IntToHex64(Cardinal(aBasePtr), 8)+' < '+IntToHex64(Cardinal(aEndPtr), 8))
...
begin
  ...
  for i := Low(flgNames) to High(flgNames) do begin
    flgNames[i] := aNames[i];
    if SameText(flgNames[i], '未使用') then
...
begin
  ...
  for i := 1 to Length(aValue) do
    case aValue[i] of
      ...
      raise Exception.Create('"'+aValue[i]+'" 不是可用的标志字符。');
    end;
...
begin
  ...
  if wbReportMode and wbReportUnknownFlags and HasUnknownFlags then begin
    WriteLn('未知标志位于：', wbDefsToPath(aParents), wbDefToName(Self));
    for i := 0 to 63 do
...
begin
  ...
  else if SameText(aValue, '默认') then
...
begin
  ...
  if Len < 4 then begin
    if wbCheckExpectedBytes then
      Result := Format('<错误：预测 %d 字节的数据，找到 %d>', [4, Len])
  end else try
    if PCardinal(aBasePtr)^ = $7F7FFFFF then
      Result := '默认'
    else begin
    ...
    if Len > 4 then
      if wbCheckExpectedBytes then
       Result := Format(' <警告：预测 %d 字节的数据，找到 %d>', [4, Len])
  except
    on E: Exception do
      Result := '$'+IntToHex64(PCardinal(aBasePtr)^, 8)+' <错误：'+E.Message+'>';
  end;
  Used(aElement, Result);
end;
...
begin
  if aValue = '' then
    ...
    s := AnsiString(aValue);
    if Length(s) <> 4 then
      raise Exception.Create('数值必须刚好是4个字符');
...
begin
  ...
  if aInt > $800 then
    Result := '['+IntToHex64(aInt,8)+'] <错误：无法处理>';
end;
...
begin
  ...
  if Assigned(aElement) then begin
    ...
    if Assigned(_File) then begin
      ...
      if FileID < 0 then
        raise Exception.Create('无法判断旧数值的 FormID');
      ...
      if FormID = aOldFormID then begin
        ...
        if NewFileID < 0 then
          raise Exception.Create('FormID ['+IntToHex64(aNewFormID, 8)+'] 引用一个文件 ' + _File.Name + ' 中找不到的 Master');
      ...
      for i := 0 to Pred(Group.ElementCount) do
        if Supports(Group.Elements[i], IwbMainRecord, Result) then begin
          ...
          if (Result.Flags._Flags and $000000C0) = $000000C0 then begin
            AV := Result.ElementNativeValues['DATA\角色数值'];
...
begin
  ...
  if Length(s) = 8 then
    ...
    if IsValid('ACVA') and SameText(Trim(aValue), '无') then begin
      Result := $FF;
      Exit;
    end else
      Result := StrToInt64('$' + aValue);
  ...
  if Assigned(aElement) then begin
    ...
    if Assigned(_File) then begin
      ...
      if NewFileID < 0 then
        raise Exception.Create('FormID ['+IntToHex64(FormID, 8)+'] 引用一个文件 ' + _File.Name + ' 中找不到的 Master');
    ...
    if CheckAll then begin
        ...
        if IsValid(MainRecord.Signature) and IsValidMainRecord(MainRecord) then begin
          ...
          if s <> '' then begin
            ...
            if not Assigned(RecordsProg) then
              RecordsProg := Wait.CreateProgress('记录', s, Pred(aFile.RecordCount) )
            else
      ...
      for i := 0 to Pred(aFile.ElementCount) do begin
        ...
        if not Assigned(GroupsProg) then
          GroupsProg := Wait.CreateProgress('群组', aFile.Elements[i].Name, Pred(aFile.ElementCount) )
        else
                ...
                if Supports(GroupRecord.Elements[j], IwbMainRecord, MainRecord) then begin
                  if IsValid(MainRecord.Signature) and IsValidMainRecord(MainRecord) then begin
                    ...
                    if s <> '' then begin
                      ...
                      if not Assigned(RecordsProg) then
                        RecordsProg := Wait.CreateProgress('记录', s, Pred(GroupRecord.ElementCount) )
                      else
  ...
  if Assigned(aElement) then begin
    ...
    if Assigned(_File) then begin
      ...
      try
        ...
        Wait := wbCreateWaitForm('正在创建下拉列表', '正在创建下拉列表，请等待...', True, 2000, 500);

        FilesProg := Wait.CreateProgress('文件', _File.Name, _File.MasterCount);
        Process(_File);
    ...
    if wbReportFormIDs then begin
      if Assigned(FoundSignatures) then
        if ClassType = TwbFormID then begin
          WriteLn('未经核查的 FormID 编制器：', wbDefsToPath(aParents), wbDefToName(Self));
      ...
      if Assigned(NotResolved) then begin
        WriteLn('表单序号含无法解析的数值：', wbDefsToPath(aParents), wbDefToName(Self));
      ...
      if Assigned(FoundNotAllowedReferences) then begin
        WriteLn('表单序号含不被允许的衍生：', wbDefsToPath(aParents), wbDefToName(Self));
...
begin
  ...
  if Assigned(aElement) then begin
    ...
    if Assigned(_File) then begin
      try
        ...
        on E: Exception do begin
          Result := '['+IntToHex64(aInt,8)+'] <错误：'+E.Message+'>';
          if wbReportMode then
            if wbReportFormIDs then begin
              if not Assigned(FoundSignatures) then
                FoundSignatures := TwbFastStringListCS.CreateSorted;
  ...
  if aInt < $800 then begin
    s := IntToHex64(aInt,8);
    Result := '['+s+'] <警告：无法处理，不过可能是被引擎给硬编译了>';
    if wbReportMode then
      if wbReportFormIDs then begin
        if not Assigned(FoundSignatures) then
          FoundSignatures := TwbFastStringListCS.CreateSorted;
    ...
    case aValue[i] of
      ...
      '0'..'9', 'a'..'f', 'A'..'F': begin
        if i = Length(aValue) then
          raise Exception.Create('非预测数值。单位数以十六进制对出现。');
        if aValue[Succ(i)] in ['0'..'9', 'a'..'f', 'A'..'F'] then begin
          Bytes[j] := StrToInt('$'+Copy(aValue,i, 2));
          Inc(j);
          Inc(i, 2);
        end else
          raise Exception.Create('"'+aValue[Succ(i)]+'" 在方位 '+IntToStr(Succ(i))+' 对 ' + GetName + ' 不是有效的特性');
      end;
    else
      raise Exception.Create('"'+aValue[i]+'" 在方位 '+IntToStr(i)+' 对 ' + GetName + ' 不是有效的特性');
    end;
  end;
...
begin
  ...
  if wbReportUnknownStrings then
    if (FoundString > 0) and (NotFoundString < 1) then begin
      WriteLn('找到 String ：', s, '：',Strings.Count,' (', FoundString, ')');
      with Strings do
        for k := 0 to Pred(Count) do
          WriteLn('  ', Strings[k], ' (', Integer(Objects[k]),')');
    end;
...
begin
  ...
  if wbReportMode then begin
    if Assigned(aElement) and (Self.noName <> '未使用') then begin
      _File := aElement._File;
...
begin
  raise Exception.Create(GetName + ' 不可编辑。');
end;
  ...
  if aInt = 0 then begin
    if IsValid('TRGT') and not IsValid('NULL') then begin
      ...
      if fidcValidRefs.IndexOf(Found) < 0 then
        Result := '找到一个 TRGT 衍生，预测：' + fidcValidRefs.CommaText;
    end else begin
      Found := 'NULL';
      if fidcValidRefs.IndexOf(Found) < 0 then
        Result := '找到一个 NULL 衍生，预测：' + fidcValidRefs.CommaText;
    end;
    Exit;
  end else if aInt = $14 then begin
    Found := 'PLYR';
    if fidcValidRefs.IndexOf(Found) < 0 then
      Result := '找到一个 PLYR 衍生，预测：' + fidcValidRefs.CommaText;
    Exit;
  end;
...
begin
  ...
  if Supports(aMainRecord.ElementByName['表单序号'], IwbContainerElementRef, Container) then
    ...
    if wbReportFormIDs and Assigned(FoundSignatures) and (FoundSignatures.Count > 0) then begin
      ...
      try
        ...
        if (s <> '') or (Sigs.Count > 0) then begin
          WriteLn('已检查的表单序号编制器存在差异：', wbDefsToPath(aParents), wbDefToName(Self));
          if s <> '' then
            WriteLn('  未找到但已通过： ', s);
          if Sigs.Count > 0 then begin
            WriteLn('  未找到但已通过： ', Sigs.CommaText);
...
begin
  raise Exception.Create(Classname + ' 不可编辑');
end;
...
begin
  ...
  if Len < Prefix then begin
    if wbCheckExpectedBytes then
      Result := Format('预测至少 %d 字节的数据，找到 %d', [Prefix , Len]);
    Exit;
  end;
  ...
  if Len < Size then begin
    if wbCheckExpectedBytes then
      Result := Format('预测 %d 字节的数据，找到 %d', [Size , Len]);
  end;
end;
...
begin
  if Assigned(aElement._File) and aElement._File.IsLocalized then begin
    if (Cardinal(aEndPtr) - Cardinal(aBasePtr)) <> 4 then
      Result := '<错误：lstring ID 应该为 Int32 数值>'
    else
      ...
      if Length(s) = 4 then begin
        ...
        if not IsAlpha then begin
          ...
          if aTransformType = ttToString then
            Result := Result + ' <警告：效果代码并不是字母数字也不是动态>';
        end;
      end else if aTransformType = ttToString then
        Result := Result + AnsiString(' <警告：预测4字节的数据但找到 ' + IntToStr(Length(s)) + '>');
    end;
    ttFromEditValue, ttFromNativeValue: begin
      ...
      if i > 0 then begin
        ...
        if MgefCode > $7FFFFF then
          raise Exception.Create('"'+t+'" 超过动态魔法效果代码的最大值');
        ...
        if (MgefCode and $80000000) <> 0 then
          raise Exception.Create('"'+Result+'" 不是可用的魔法效果代码');
