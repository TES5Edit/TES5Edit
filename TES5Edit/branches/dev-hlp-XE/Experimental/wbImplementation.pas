...
function TwbMainRecord.GetEditValue: Variant;
begin
  if wbDisplayLoadOrderFormID then
...
procedure TwbMainRecord.SetEditValue(const aValue: Variant);
begin
  if not wbEditAllowed then
    raise Exception.Create(GetName + ' can not be edited.');
...
function TwbSubRecord.GetEditValue: Variant;
var
  SelfRef : IwbContainerElementRef;
begin
  SelfRef := Self as IwbContainerElementRef;
...
procedure TwbSubRecord.SetEditValue(const aValue: Variant);
var
  SelfRef : IwbContainerElementRef;
...
function TwbElement.GetEditValue: Variant;
begin
  Result := '';
end;
...
procedure TwbElement.SetEditValue(const aValue: Variant);
begin
  raise Exception.Create(GetName + ' can not be edited.');
end;
...
procedure TwbValue.SetEditValue(const aValue: Variant);
var
  TestValue, OldValue, NewValue: Variant;
begin
  if not wbEditAllowed then
    raise Exception.Create(GetName + ' can not be edited.');

  TestValue := GetEditValue;
  if (TestValue = Unassigned) or (aValue <> TestValue) then begin
    OldValue := GetNativeValue;
...
function TwbFlag.GetEditValue: Variant;
var
  s: string;
begin
...
procedure TwbFlag.SetEditValue(const aValue: Variant);
var
  s: string;
  c: Char;
begin
...
function TwbValueBase.GetEditValue: Variant;
var
  SelfRef : IwbContainerElementRef;
begin
  SelfRef := Self as IwbContainerElementRef;
...
procedure TwbValueBase.SetEditValue(const aValue: Variant);
var
  OldValue, NewValue: Variant;
begin
  if not wbEditAllowed then
...
procedure TwbStringListTerminator.SetEditValue(const aValue: Variant);
begin
end;
