...
procedure AddRequiredMasters(aMasters: TStrings; const aTargetFile: IwbFile); forward;
...
//      Target.ReportRequiredMasters(sl, aAsNew);
...
begin
  ...
  for i := Low(cntElements) to High(cntElements) do begin
    ...
    for l := Low(cntElements) to High(cntElements) do begin
      if cntElements[l].MemoryOrder = Low(Integer) then
        cntElements[l].MemoryOrder := Pred(Length(cntElements));
...
//              Element := TwbUnion.Create(uContainer, BasePtr, nil, UnionDef, '');
//              Element.SortOrder := 0;
//              Element.MemoryOrder := 0;
            end;
...
//          if Supports(Container, IwbDataContainer, dContainer) then
//            Move(dContainer.DataBasePtr^, dcDataBasePtr^, dContainer.DataSize);
