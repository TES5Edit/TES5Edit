...
const
  VersionString               = '3.0.10 (2010-10-25) EXPERIMENTAL';
...
type
  TwbGameMode = (gmTES4, gmFO3, gmFNV);
...
function wbBeginInternalEdit: Boolean;
...
function wbBeginInternalEdit: Boolean;
begin
  Result := wbAllowInternalEdit and not _BlockInternalEdit;
  if Result then
...
type
  ...
  protected
    ...
    function FindSparseName(aSearchIndex: Integer; var Index: Integer): Boolean;
...
begin
  ...
  try
    ...
    for i := Low(enSparseNames) to High(enSparseNames) do begin
      Assert(aSparseNames[     i * 2 ].VType = vtInteger);
...
function TwbEnumDef.FindSparseName(aSearchIndex: Integer; var Index: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
  Result := False;
  ...
  while L <= H do begin
    I := (L + H) shr 1;
    C := CmpI32(enSparseNamesMap[I].snIndex, aSearchIndex);
    if C < 0 then
      L := I + 1
    else begin
      H := I - 1;
