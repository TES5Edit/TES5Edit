...
namespace RTF
{
    ...
    public abstract class RTFBuilderbase : IDisposable
    {
        ...
        public ushort DefLang { [DebuggerStepThrough]
        ...
        protected abstract IEnumerable<RTFBuilderbase> EnumerateCellsInternal(RTFRowDefinition rowDefinition,
        ...
        {
            int idx = IndexOf(linkColor);
            //this.AppendRTFInternal(string.Format(@"{\field {\*\fldinst HYPERLINK ""{0}""}{\fldrslt\cf{1}\ul {2}}}", hyperlink, idx, text));
            AppendRTFInternal(@"{\field {\*\fldinst HYPERLINK " + '\"' + hyperlink + '\"' + @"} {\fldrslt\cf" +
                              idx.ToString() + @"\ul " + text + @"}}");
            return this;
        }
        ...
        private string GetColoursFromRtf(string rtf)
        {
            var RegexObj =
                new Regex(
                    "\\{\\\\colortbl ;(?<colour>\\\\red(?<red>\\d{0,3})\\\\green(?<green>\\d{0,3})\\\\blue(?<blue>\\d{0,3});)*\\}");
        ...
        private string GetFontsFromRtf(string rtf)
        {
            ...
            var RegexObj =
                new Regex(
                    @"\{\\fonttbl(?<raw>\{\\f(?<findex>\d{1,3}) ?\\(?<fstyle>fswiss|fnil|froman|fmodern|fscript|fdecor|ftech|fbidi) ?(?<fprq>\\fprq\d{1,2} ?)?(?<fcharset>\\fcharset\d{1,3})? (?<FONT>[\w -]+);\})+\}\r?\n",
        ...
        public IEnumerable<RTFBuilderbase> EnumerateCells(RTFRowDefinition rowDefinition,
        ...
        [DebuggerStepThrough]
        public RTFBuilderbase PrependLineFormatIf(string format, ref bool appended, object arg0, object arg1,
                                                  object arg2)
        {
            string formated = string.Format(format, arg0, arg1, arg2);
            return PrependLineIf(formated, ref appended);
        ...
        {
            return TextRenderer.MeasureText(text, _sysfont);
        }
    }
}