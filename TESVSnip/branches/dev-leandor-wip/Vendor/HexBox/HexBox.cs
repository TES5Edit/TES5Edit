    ...
    internal class HexBox : Control
    {
        ...
        private class KeyInterpreter : IKeyInterpreter
        {
            ...
            public virtual bool PreProcessWmChar(ref Message m)
            {
                ...
                if (Uri.IsHexDigit(c))
                {
                    ...
                    string sCb = currentByte.ToString("X", Thread.CurrentThread.CurrentCulture);
                    if (sCb.Length == 1)
                        sCb = "0" + sCb;
                    ...
                    byte newcb = byte.Parse(sNewCb, NumberStyles.AllowHexSpecifier, Thread.CurrentThread.CurrentCulture);
                    if (isInsertMode)
        ...
        private class StringKeyInterpreter : KeyInterpreter
        {
            ...
            public override bool PreProcessWmChar(ref Message m)
            {
                ...
                if (isInsertMode)
                    _hexBox._byteProvider.InsertBytes(pos, new[] {(byte) c});
                else
        ...
        private void UpdateScrollSize()
        {
            ...
            {
                var scrollmax = (long) Math.Ceiling(_byteProvider.Length/(double) _iHexMaxHBytes - _iHexMaxVBytes);
        ...
        private void PaintLineInfo(Graphics g, long startByte, long endByte)
        {
            ...
            Color lineInfoColor = (LineInfoForeColor != Color.Empty) ? LineInfoForeColor : ForeColor;
