namespace TESVSnip.UI.Forms
{
    ...
    internal partial class MainView
    {
        public void giveSelectionNewFormID(bool updateReference)
        {
            ...
            if (formCount > 0)
            {
                this.SendStatusText(string.Format("Changed {0} FormID's and updated {1} references", formCount, refCount), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No FormID's changed", Color.OrangeRed);
            }
        }
        ...
        private void cleanEspToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                this.cleanRecurse2(plugin.Masters[plugin.Masters.Length - 1].Records[j] as Rec, ref count, lookup);
            }
            ...
            {
                MessageBox.Show(string.Empty + count + " records removed");
            }
        ...
        private void cleanUnusedStringsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                this.SendStatusText(string.Format("Cleaned {0} unused strings from table", count), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No unused strings to clean", Color.OrangeRed);
            }
        }
        ...
        private void copyReferencedStringsFromMastersToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                this.SendStatusText(string.Format("Copied {0} strings from master", count), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No strings to copy from master", Color.OrangeRed);
            }
        }
        ...
        private void createStubsForMissingStringsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                this.SendStatusText(string.Format("Created {0} strings stubs", count), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No strings needed stubs", Color.OrangeRed);
            }
        }
        ...
        private void dumpEDIDListToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (this.SaveEdidListDialog.ShowDialog() != DialogResult.OK)
            {
                return;
            }

            var sw = new StreamWriter(this.SaveEdidListDialog.FileName);
            if (this.PluginTree.SelectedRecord is Plugin)
            {
                foreach (Rec r in this.PluginTree.SelectedRecord.Records)
                {
                    this.DumpEdidsInternal(r, sw);
                }
            }
            else
            {
        ...
        private void extractInternalStringsToTableToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                this.SendStatusText(string.Format("Copied {0} strings to table", count), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No strings to copied to table", Color.OrangeRed);
            }
        }
        ...
        private void findDuplicatedFormIDToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            foreach (var record in plugin.Enumerate().OfType<Record>())
            {
                if (ids.ContainsKey(record.FormID))
                {
                    ...
                    this.PluginTree.SelectedRecord = record;
                    MessageBox.Show("Record duplicates " + ids[record.FormID].DescriptiveName);
                    this.SendStatusText(string.Format("Record FormID duplicates {0}", ids[record.FormID].DescriptiveName), Color.OrangeRed);
                    return;
                }
                else
                {
            ...
            ids.Clear();
            this.SendStatusText("No duplicate FormID's found", SystemColors.ControlText);
        }
                        ...
                        case "LVLI":
                            for (int i = 0; i < r.SubRecords.Count; i++)
                            {
                                ...
                                else if (r.SubRecords[i].Name == "LVLO")
                                {
                                    ...
                                    sb3.AppendLine(
                                        "      <LVLO level=\"" + TypeConverter.h2ss(data[0], data[1]) + "\" formid=\"" + (formid & 0xffffff).ToString("X6") + "\" count=\""
                                ...
                                foreach (var lstring in lstrings.Strings)
                                {
                                    ...
                                    switch (lstring.Type)
                                    {
                                        case LocalizedStringFormat.Base:
                                            plugin.Strings[lstring.ID] = lstring.Value;
                                            break;
                                        case LocalizedStringFormat.DL:
                                            plugin.DLStrings[lstring.ID] = lstring.Value;
                                            break;
                                        case LocalizedStringFormat.IL:
                                            plugin.ILStrings[lstring.ID] = lstring.Value;
                                            break;
                                    }
                                }

                                this.SendStatusText(string.Format("Imported {0} strings", lstrings.Strings.Length), SystemColors.ControlText);
                            }
                            else
                            {
                                this.SendStatusText("No strings internalized", Color.OrangeRed);
                            }
                        }
                    }
                }
        ...
        private void internalizeStringReferencesToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (count > 0)
            {
                this.SendStatusText(string.Format("Internalized {0} strings", count), SystemColors.ControlText);
            }
            else
            {
                this.SendStatusText("No strings internalized", Color.OrangeRed);
        ...
        {
            var records = this.PluginTree.SelectedRecords.SelectMany(rec => rec.Enumerate(x => true).OfType<Record>().Distinct()).ToList();
            ...
            finally
            {
                ...
                this.RebuildSelection();
                this.SendStatusText(string.Format("Reordered SubRecords on {0} records", records.Count), SystemColors.ControlText);
            }
        }
        ...
        private void sanitizeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var rec = this.PluginTree.SelectedRecord as Record;
            ...
            {
                this.PluginTree.RebuildObjects();
                this.SendStatusText("Sanitization Complete", SystemColors.ControlText);
            }
        }
