...
type
  TwbSaveDefTES5Base = class(TwbSaveDef)
  protected
    sdSaveSexEnum      : IwbEnumDef;
    sdPropTypeEnum     : IwbEnumDef;
...
procedure TwbSaveDefTES5Base.DefineTES5SavesA;
begin
  sdPropTypeEnum := wbEnum([
    {00} 'None',
    {01} 'Object',
    {02} 'String',
    {03} 'Int32',
  ...
  sdSaveSexEnum := wbEnum(['Male','Female']);
...
begin
  ...
  if not Assigned(lTables) or (lTables.VMObjectArrayCount<0) then begin
    ...
    Result := lTables.VMObjectArrayCount+lTables.VMObjectDetachedArrayCount;
end;
...
function TwbSaveDefTES5Base.ChangedFormGetChapterTypeName(aBasePtr: Pointer; aEndPtr: Pointer; const aElement: IwbElement): string;
var
  aType : Integer;
begin
  ...
  if Supports(Element, IwbDataContainer, Container) then begin
    ...
    if Assigned(Element) then begin
      ...
      if (Result >= 5) and (Result <= 11) then begin
        ...
        if lSaveContext is TwbSaveContextTES5 then
          TwbSaveContextTES5(lSaveContext).LastRegistrationStart := Result;
      end;
    end;
  end;
end;
...
begin
  ...
  if not Assigned(aElement) then Exit;
  var lSaveContext := aElement.SaveContextObj;
  if not (lSaveContext is TwbSaveContextTES5) then Exit;
  case TwbSaveContextTES5(lSaveContext).LastRegistrationStart of
...
procedure TwbSaveDefTES5Base.DefineTES5SavesS;  // This is all based on current UESP, and HexDump, Triria TESSaveLib and the Runtime
var
...
begin
  ...
  wbVariable := wbStruct('Variable', [
    wbInteger('Type', itU8, sdPropTypeEnum),
  ...
  wbArrayTableEntry := wbStruct('Array Entry Data', [  // UESP: arrayInfo
    wbInteger('Array Handle', itU32),
    wbInteger('Array Type', itU8, sdPropTypeEnum),    // valid values : 0 to 5, 0B to 0F
...
// included in ExtraDataList  wbUnionCHANGE_ACTOR_LEVELED_ACTOR := wbUnion('Leveled Actor', ChangedFlag18Decider, [wbNull, wbNull]);
  ...
  wbUnionCHANGE_NPC_GENDER := wbUnion('Gender', ChangedFlag24Decider, [wbNull, wbInteger('Change Actor Gender', itU8, sdSaveSexEnum)]);
...
//  wbUnionCHANGE_ACTOR_PERMANENT_MODIFIERS := wbUnion('Permanent Modifiers', ChangedFlag23Decider, [wbNull, wbNull]);
  ...
  wbHeader := wbStruct('Header', [
    ...
    wbInteger('Player Sex', itU16, sdSaveSexEnum),
...
procedure TwbSaveDefTES5Base.Define;
begin
  FilePlugins := 'Plugins';
  DefineTES5SavesA;
  DefineTES5SavesS;
...
procedure TwbSaveDefTES5.Define;
begin
  inherited;
  FileExtension := '.ess';
...
procedure TwbCoSaveDefTES5.Define;
begin
  inherited;
  FileExtension := '.skse';
  FileMagic := 'SKSE';
...
initialization
  wbRegisterSaveDefs([gmTES5, gmTES5VR, gmEnderal, gmSSE, gmEnderalSE], TwbSaveDefTES5, TwbCoSaveDefTES5, TwbSaveContextTES5);
end.
